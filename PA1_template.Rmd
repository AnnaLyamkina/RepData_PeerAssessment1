...
Therefore, mean total number of steps per day is `r format_mean_tspd` and median total number of steps per day is `r format_median_tspd`
...
```
Therefore, the maximum steps were taken in `r max_aspi` interval.
...
Therefore, mean total number of steps per day for NA corrected data frame is `r format_mean_tspd_na` and median total number of steps per day is `r format_median_tspd_na` to compare with previous values of `r format_mean_tspd` and  `r format_median_tspd`. As we see, mean and median values did not visibly change. The only significant difference for histograms is frequency of the mean total steps value as we replaced NA with effectively these data.
